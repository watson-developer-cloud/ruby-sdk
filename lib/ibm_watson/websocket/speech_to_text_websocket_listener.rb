...
class WebSocketClient
  def initialize(audio: nil, chunk_data:, options:, recognize_callback:, url:, headers:, disable_ssl:)
    @audio = audio
    @options = options
  ...
  def start
    ...
    EM.run do
      if @disable_ssl
        @client = Faye::WebSocket::Client.new(@url, tls: { verify_peer: false }, headers: @headers)
      else
