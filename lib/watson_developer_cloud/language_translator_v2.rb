...
class LanguageTranslatorV2 < WatsonService
  ...
  # @param text [Array[String]]  Input text in UTF-8 encoding. Multiple entries will result in multiple translations
  #   in the response.
  # @param model_id [String]  Model ID of the translation model to use. If this is specified, the **source** and
  #   **target** parameters will be ignored. The method requires either a model ID or both
  ...
  #   to select a translation model. When `source` and `target` are set, and a model ID is
  #   not set, the system chooses a default model for the language pair (usually the model
  #   based on the news domain).
  # @param target [String]  Language code of the translation target language. Use with source as an alternative
  ...
  #   (default and non-default) for each language pair. To return only default models, set
  #   this to `true`. To return only non-default models, set this to `false`.
  # @return [DetailedResponse] A `DetailedResponse` object representing the response.
  ...
  #
  #   Depending on the size of the file, training can range from minutes for a glossary to
  #   several hours for a large parallel corpus. Glossary files must be less than 10 MB.
  #   The cumulative file size of all uploaded glossary and corpus files is limited to 250
  #   MB.
  ...
  # @param name [String]  An optional model name that you can use to identify the model. Valid characters are
  #   letters, numbers, dashes, underscores, spaces and apostrophes. The maximum length is
  #   32 characters.
  ...
  #   phrase translations. You can upload only one glossary with a file size less than 10
  #   MB per call.
  # @param parallel_corpus [File]  A TMX file that contains entries that are treated as a parallel corpus instead of a
  #   glossary.
  # @param monolingual_corpus [File]  A UTF-8 encoded plain text file that is used to customize the target language model.
