...
class WatsonDeveloperCloud::PersonalityInsightsV3 < WatsonService
  ...
  #   returns its results in a variety of languages. You can provide plain text, HTML, or
  #   JSON input by specifying the **Content-Type** parameter; the default is
  #   `text/plain`. Request a JSON or comma-separated values (CSV) response by specifying
  #   the **Accept** parameter; CSV output includes a fixed number of columns and optional
  #   headers.
  #
  #   Per the JSON specification, the default character encoding for JSON content is
  #   effectively always UTF-8; per the HTTP specification, the default encoding for plain
  #   text and HTML is ISO-8859-1 (effectively, the ASCII character set). When specifying
  #   a content type of plain text or HTML, include the `charset` parameter to indicate
  #   the character encoding of the input text; for example: `Content-Type:
  ...
  # @param accept [String]  The type of the response: application/json or text/csv. A character encoding can be
  #   specified by including a `charset` parameter. For example, 'text/csv;charset=utf-8'.
  ...
  #   **Content-Language** is the only way to specify the language. When **Content-Type**
  #   is `application/json`, **Content-Language** overrides a language specified with the
  #   `language` parameter of a `ContentItem` object, and content items that specify a
  #   different language are ignored; omit this parameter to base the language on the
  #   specification of the content items. You can specify any combination of languages for
  #   **Content-Language** and **Accept-Language**.
  # @param accept_language [String]  The desired language of the response. For two-character arguments, regional variants
  ...
  # @param raw_scores [Boolean]  Indicates whether a raw score in addition to a normalized percentile is returned for
  #   each characteristic; raw scores are not compared with a sample population. By
  ...
  #   `text/csv`.
  # @param consumption_preferences [Boolean]  Indicates whether consumption preferences are returned with the results. By default,
  #   no consumption preferences are returned.
