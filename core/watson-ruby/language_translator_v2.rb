...
=end
  ...
  def translate(text:, model_id: nil, source: nil, target: nil, **kwargs)
=begin
    Translate.

    Translates the input text from the source language to the target language.

    :param list[String] text: Input text in UTF-8 encoding. Multiple entries will result in multiple translations in the response.
    :param String model_id: Model ID of the translation model to use. If this is specified, the **source** and **target** parameters will be ignored. The method requires either a model ID or both the **source** and **target** parameters.
    :param String source: Language code of the source text language. Use with `target` as an alternative way to select a translation model. When `source` and `target` are set, and a model ID is not set, the system chooses a default model for the language pair (usually the model based on the news domain).
    :param String target: Language code of the translation target language. Use with source as an alternative way to select a translation model.
...
=end
  ...
  def list_identifiable_languages(**kwargs)
=begin
...
=end
  ...
  def list_models(source: nil, target: nil, default_models: nil, **kwargs)
=begin
    List models.
    ...
    :param Boolean default_models: If the default parameter isn't specified, the service will return all models (default and non-default) for each language pair. To return only default models, set this to `true`. To return only non-default models, set this to `false`.
    :param Hash headers: A `Hash` containing the request headers
    :return: A `DetailedResponse` object representing the response.
    :rtype: DetailedResponse
=end
  ...
  def create_model(base_model_id:, name: nil, forced_glossary: nil, parallel_corpus: nil, monolingual_corpus: nil, forced_glossary_filename: nil, parallel_corpus_filename: nil, monolingual_corpus_filename: nil, **kwargs)
=begin
    Create model.
    ...
    :param String base_model_id: The model ID of the model to use as the base for customization. To see available models, use the `List models` method.
    :param String name: An optional model name that you can use to identify the model. Valid characters are letters, numbers, dashes, underscores, spaces and apostrophes. The maximum length is 32 characters.
